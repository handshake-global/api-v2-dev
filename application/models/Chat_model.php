...
class Chat_model extends CI_Model {
	...
	public function getConnections($data=[]){
		if(empty($data))
			...
			$sentMgs = $this->db->query("SELECT tbl.messageId,tbl.message AS lastMessage,tbl.file AS fileUrl,tbl.createdAt AS lastMessageTime,tbl.status,'sent' AS 'msgType',`users`.`userId`,concat(users.firstName,' ',users.lastName) as userName,`users`.`avatar` as `userPhoto`,`users`.loggedIn as `isLogin`,`user_details`.`designation`, profile.connections FROM (SELECT * FROM messages WHERE `sender`=".$data['userId']." GROUP BY messageId ORDER BY messageId DESC) AS tbl,users,user_details,profile WHERE users .userId=tbl.receiver and user_details.userId = tbl.receiver and profile.userId = tbl.receiver GROUP BY tbl.receiver ORDER BY messageId DESC")->result_array();
			...
			if(!empty($receivers))			
				$receivedMsgs = $this->db->query("SELECT tbl.messageId,tbl.message AS lastMessage,tbl.file AS fileUrl,tbl.createdAt AS lastMessageTime,tbl.status,'received' AS 'msgType',`users`.`userId`,concat(users.firstName,' ',users.lastName) as userName,`users`.`avatar` as `userPhoto`,`users`.loggedIn as `isLogin`,`user_details`.`designation`, profile.connections FROM (SELECT * FROM messages WHERE `receiver`=".$data['userId']." AND `sender` IN (".implode(',',$receivers).") GROUP BY messageId ORDER BY messageId DESC) AS tbl,users,user_details,profile WHERE users .userId=tbl.sender and user_details.userId = tbl.sender and profile.userId = tbl.sender  GROUP BY tbl.sender ORDER BY messageId DESC")->result_array(); 	
